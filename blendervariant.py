...
class Point:
    ...
    def getThetaDegrees(self):
        returnvar = self.theta * 360 / (2 * np.pi)
        print(returnvar)
        return returnvar
...
def generateCubes(pointlist):
    for point in pointlist:
        zrot = point.getTheta()
        bpy.ops.mesh.primitive_plane_add(location=(point.getX(), point.getY(), 0.0), radius=point.getSideLength() / 2,
                                         rotation=(0, 0, zrot))


point1 = Point(0, 0, 0, 5)
listpoints = generatePoints(point1, 10)
generateCubes(listpoints)
